...
@router.get(
    "/vehiculos",
    response_model=list[schemas.Vehiculo]
)
...
@router.get(
    "/vehiculos/usuario/{usuario_id}",
    response_model=list[schemas.Vehiculo]
)
def obtener_vehiculos_usuario(
    usuario_id: int,
    db: Session = Depends(get_db)
):
...
@router.post(
    "/servicios",
    response_model=schemas.ServicioHistorico
)
def crear_historial(
    data: schemas.ServicioHistoricoCreate,
    db: Session = Depends(get_db)
):
    ...
    if not vehiculo:
        raise HTTPException(
            status_code=404,
            detail="Vehículo no existe"
        )

    nuevo = models.ServicioHistorico(
        **data.dict()
    )
...
@router.get(
    "/vehiculos/{vehiculo_id}/servicios",
    response_model=list[schemas.ServicioHistorico]
)
